...
function generateId(page) {
    return (0, common_2.generatePageId)((0, common_2.getPageTypeV2)(page.component.name), page.entitySet);
}
...
function getPagesAndConfigs(v2Pages, flex, manifest, schemas, fragments, logger) {
    ...
    for (const v2PageName in v2Pages) {
        ...
        if (jsonSchema) {
            if (page.pageType === ux_specification_types_2.PageTypeV2.ListReport) {
            ...
            else if (page.pageType === ux_specification_types_2.PageTypeV2.AnalyticalListPage) {
                const importParameters = {
                    manifest,
                    flex,
                    jsonSchema: schemas[jsonSchema],
...
class V2AppProvider extends appProvider_1.AppProvider {
    constructor(importParameters) {
        (0, i18n_1.initI18n)();
        let v2Pages;
        if (!importParameters.manifest[ux_specification_types_2.ManifestSection.generic]) {
            ...
            v2Pages = importParameters.manifest[ux_specification_types_2.ManifestSection.generic].pages;
        }
        if (!v2Pages) {
            (0, extensionLogger_1.log)(importParameters.logger, {
                severity: "error" /* LogSeverity.Error */,
        ...
        const pageId = v2Pages && generateId(v2Pages[pageKey]);
        // Import settings
        const appSettings = new application_1.AppSettings();
        const schemaFile = (0, common_2.getSchemaFilePath)(ux_specification_types_2.SchemaType.Application);
        ...
        // Call base
        super(importParameters.manifest, importParameters.appSchemas, pageId, getPagesAndConfigs(v2Pages, importParameters.flexChanges, importParameters.manifest, importParameters.appSchemas, importParameters.fragments, importParameters.logger), ux_specification_types_2.FioriElementsVersion.v2, appSettings);
    }
}
...
function getPages(v2Pages, componentName) {
    ...
    for (const v2PageName in v2Pages) {
        ...
        if (componentName === ux_specification_types_2.v2.FE_TEMPLATE_V2_OBJECT_PAGE && v2Page.pages) {
